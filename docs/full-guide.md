...
| `HISTORICAL_SOURCE_PRIORITY` | 历史行情数据源优先级（逗号分隔），如 `efinance,akshare,baidu,tushare,pytdx,baostock,yfinance,longbridge`。留空则使用内置优先级；配置 `TUSHARE_TOKEN` 后自动将 tushare 提到最前 | 见 .env.example | 可选 |
